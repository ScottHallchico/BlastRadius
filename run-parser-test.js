...
test('REMOVE UserAvatar', "Remove the legacy UserAvatar component.", (res) => {
    ...
    if (res.target.type !== 'component') throw new Error('Target type mismatch');
    if (!res.changeSemantics.includes('legacy')) throw new Error('Semantics missing legacy');
});

test('Ambiguous', "Refactor something completely ambiguous.", (res) => {
    if (res.operation !== 'MODIFY') throw new Error('Op mismatch');
    if (res.target.name !== 'Refactor') throw new Error('Target name mismatch');
    if (res.target.type !== 'unknown') throw new Error('Target type mismatch');
});
