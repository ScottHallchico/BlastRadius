export function parseChangeDescription(description) {
    ...
    const determineType = (name, desc) => {
        const lowerName = name.toLowerCase();
        const lowerDesc = desc.toLowerCase();
        if (lowerName.includes('redis') || lowerName.includes('kafka') || lowerName.includes('database') || lowerName.includes('publisher'))
            return 'infrastructure';
        if (lowerName.includes('component') || new RegExp(`${name}\\s+component`, 'i').test(desc))
            return 'component';
        if (lowerName.includes('service') || new RegExp(`${name}\\s+service`, 'i').test(desc))
            return 'service';
        if (lowerName.includes('helper') || lowerName.includes('function') || lowerName.includes('method') || new RegExp(`${name}\\s+(?:helper|function|method)`, 'i').test(desc))
            return 'function';
        if (lowerName.includes('timeout') || lowerName.includes('config') || new RegExp(`${name}\\s+(?:config|client)`, 'i').test(desc))
            return 'config';
        ...
        if (lowerDesc.includes('service'))
            return 'service';
        if (lowerDesc.includes('redis') || lowerDesc.includes('kafka'))
            return 'infrastructure';
        if (lowerDesc.includes('client') || lowerDesc.includes('api'))
            return 'config';
        return 'unknown';
    };
    // 2. Semantics mapping
    if (lowerDesc.includes('async'))
        spec.changeSemantics.push('async');
    if (lowerDesc.includes('handler') || lowerDesc.includes('prop'))
        spec.changeSemantics.push('props', 'handler');
    if (lowerDesc.includes('legacy') || lowerDesc.includes('deprecation'))
        spec.changeSemantics.push('legacy', 'deprecation');
    if (lowerDesc.includes('event') || lowerDesc.includes('publisher') || lowerDesc.includes('kafka'))
        spec.changeSemantics.push('event', 'message-broker');
    if (lowerDesc.includes('timeout') || lowerDesc.includes('client') || lowerDesc.includes('network'))
        spec.changeSemantics.push('network', 'configuration');
    if (lowerDesc.includes('color') || lowerDesc.includes('style') || lowerDesc.includes('css') || lowerDesc.includes('classname'))
        spec.changeSemantics.push('style', 'visual');
    if (lowerDesc.includes('doc') || lowerDesc.includes('readme') || lowerDesc.includes('documentation'))
        spec.changeSemantics.push('documentation');
