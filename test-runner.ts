...
async function runTest() {
  ...
  try {
    ...
    let passed = true;
    if (result.status !== 'complete') passed = false;
    if (result.riskLevel !== 'HIGH') passed = false;
    if (result.componentsAffected <= 0) passed = false;
    if (result.implicitCouplings !== 1) passed = false;
    if (result.invariantsViolated !== 1) passed = false;
